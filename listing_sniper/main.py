...
async def handle_new_listing(exchange: str, pair: str, base: str, session, bot: Bot, client: httpx.AsyncClient):
    logger.info(f"NEW LISTING — {exchange.upper()}: {pair} ({base})")
    ...
    if not poly_addr:
        await alert(bot,
            ...
            f"Exchange: <b>{exchange.upper()}</b> | Pair: <b>{pair}</b>\n"
            f"Token <b>{base}</b> not found on Polygon — monitor manually."
        )
        return
    ...
    if tx:
        ...
        await alert(bot,
            f"⚠️ <b>BUY FAILED</b> — {base}\n"
            f"Check USDC balance or pool liquidity.\n"
            f"Pair: <b>{pair}</b> on {exchange.upper()}"
        )
...
async def run():
    ...
    logger.info("CEX Listing Sniper started — monitoring Coinbase, Kraken, Bybit, OKX (60s interval)")
            ...
            try:
                for exchange_name, poller in POLLERS.items():
                    ...
                    for pair in current_pairs:
                        ...
                        await asyncio.sleep(2)  # avoid CoinGecko rate limit
